...
for creator,content in posts:
    ...
    # 1–3 issues → CLEANED
    # >3 issues → BLOCKED
    clean_p = Post_TextCleaned(content, Banned_Words)
    if issue_count==0:
        status="SAFE"
        final_p=clean_p
    elif issue_count<=3:
    ...
    # extract links
    Links_Extracted=extract_links(content)
...
for u in users:
    print(u,":",users[u])
print("\n----------------Final Report -------------------")
print(f"Total Posts Screened: {Total_p} | Cleaned: {Cleaned_p} | Blocked: {Blocked_p}")